/* eslint-env browser */
import ScrollBehavior, { TransitionHook, ScrollPosition } from 'scroll-behavior'
...
export default class RestoreScroll<Action extends FluxStandardRoutingAction> {
      ...
      shouldUpdateScroll: (request) => {
        ...
        if (typeof requested === 'number') {
          ...
          return entryToRestore
            ? this.readScrollPosition(entryToRestore, null) || true
            : true
        }
        return requested
      },
  ...
  saveScroll: Middleware<Action> = (api: Api<Action>) => {
    if (isServer()) {
